...
function initDistanceDFS ($, $node, index, depth = 0) {
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    if (!tagsToIgnore.has(tagName)) {
      nextIndex = initDistanceDFS($, $(child), nextIndex + 1, depth + 1);
    }
  }

  const next = $node.next().get(0);
  if (next) {
    return initDistanceDFS($, $(next), nextIndex + 1 + $node.text().length, depth + 1);
  } else {
